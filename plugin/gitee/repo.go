...
// GetRepo 返回认证用户下的仓库
// 参考 https://gitee.com/api/v5/swagger#/getV5ReposOwnerRepo
func (h *holder) GetRepo() (*plugin.Repo, error) {
	...
	log.Debug().Str("GET URL", url).Msg("Get repo")
...
// 参考 https://gitee.com/api/v5/swagger#/postV5UserRepos
func (h *holder) CreateRepo() (*plugin.Repo, error) {
	...
	log.Debug().Str("POST URL", url).Str("Request Body", string(reqBody)).Msg("Create repo")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nil, err
	}
...
// 参考 https://gitee.com/api/v5/swagger#/getV5UserStarred
func (h *holder) FetchStarredRepos(pg *plugin.Pagination) (items []*plugin.Repo, err error) {
	...
	log.Debug().Str("GET URL", url).Msg("Fetch repos being starred")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nil, err
	}
